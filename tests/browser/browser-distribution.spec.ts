...
test('the documented plain HTML example renders against the built browser distribution', async ({ page }) => {
  ...
  const examples = new Map(
    [...documentation.matchAll(/^```html example=([\w-]+)\r?\n([\s\S]*?)^```/gm)].map((match) => [match[1], match[2]])
  )
  expect(examples.has('browser-basic')).toBeTruthy()
  expect(examples.has('browser-url')).toBeTruthy()
  const errors: string[] = []
  ...
  await page.setContent(examples.get('browser-basic')!)
  await page.getByRole('button', { name: '计数 0', exact: true }).click()
  await expect(page.getByRole('button', { name: '计数 1', exact: true })).toBeVisible()
  await page.goto('about:blank')
  await page.setContent(examples.get('browser-url')!)
