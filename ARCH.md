...
AnyDrop 是最终样式蓝本；参考之间的颜色、字体和组件外观冲突，应直接修正 CakeUI 与 AnyDrop 保持一致。粉色主题与紧凑密度承载这套样式。
...
- 编码规范以 `AGENTS.md` 为当前仓库入口。原外部《样式和编码通常规则.md》已由维护者提供，核心约定（具名函数、TypeScript + SCSS、完整类名、原生布局和组件目录）已收录其中。
